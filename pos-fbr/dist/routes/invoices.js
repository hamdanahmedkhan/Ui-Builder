...
router.post('/', async (req, res) => {
    ...
    const pricedLines = lines.map((l) => {
        ...
        if (!product)
            throw new Error(`Product ${l.productId} not found`);
    ...
    const totals = computeTotals(pricedLines.map((pl) => ({ productId: pl.product.id, quantity: pl.quantity, unitPrice: pl.unitPrice, taxRatePercent: pl.taxRatePercent })), discount || 0);
    ...
    await prisma.fbrLog.create({ data: { receiptId: created.id, success: fbrResult.success, statusCode: fbrResult.success ? 200 : 500, requestBody: fbrPayload, responseBody: fbrResult.raw } });
    // PDF
